...
/** The form controls an auth screen can pin a server message onto, by field name. */
export type AuthControls = Partial<Record<string, FieldTree<string>>>;
...
/**
 * Split a rejected auth submit into the two things the sign-in and sign-up
 * screens do with it, identically:
 *
 ...
 * control (ADR 0002: the adapter has already reduced every failure shape to one
 * display-ready `ApiError`, so nothing here branches on transport detail).
 */
export function partitionAuthError(
  error: unknown,
