...
import { TransactionSplitFailure, TransactionSplitPayload } from '../../data/linked-contributions/transaction-split';
...
export class TransactionSplitDialog {
  ...
  constructor() {
    ...
    effect(() => {
      if (this.recoveryState().status === 'confirmed') {
  ...
  protected removeRow(index: number): void {
    if (this.model().contributions.length === 1 || this.formLocked()) {
      return;
    }
  ...
  protected targetOverrun(index: number): TargetOverrun | null {
    ...
    if (row?.amount === null || row?.amount === undefined || goal === null) {
      return null;
    }
    const proposedProgress = sumPesos([goal.currentAmount, row.amount]);
    if (proposedProgress <= goal.targetAmount) {
      return null;
    }
    return {
      goal,
