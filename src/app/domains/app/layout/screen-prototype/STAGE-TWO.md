...
The user chose A for Accounts, Transactions, Schedules, Categories, Tags, Profile and sign-in, and B for Account/Goal detail. Budgets and Goals list use compact cards across the available content width, not a narrow centered column. Sign-up inherits A; recovery and confirmation use compact centered cards. Sidebar headings and the phone More panel are approved. `variant=selected` follows that mix. The initial recommendation of C for Transactions is superseded.

The compact planning revision uses 16px card padding/gutters, tighter vertical spacing, 20px amounts, and an auto-fitting grid with a 320px preferred minimum card width (one full-width column on small phones). There is no content max-width cap for these two screens. Explicit A/B/C retain their original structures for comparison. This is a concrete interpretation of the user's compact/full-width direction, awaiting visual feedback rather than a separately approved exact spacing specification.
...
| Budgets              | Compact full-width cards selected; revision to review | Current/Future/Finished, current Cycle dates, Spent versus ceiling, overspending, future without progress, finished under/over phrasing                    |
| Goals list           | Compact full-width cards selected; revision to review | Active/Completed/Abandoned, progress/target, funded and overdue examples, lifecycle menu placement, detail entry                                           |
...
- Compact planning revision: visually checked full-width Light Budgets and Dark Goals on desktop, and Dark Goals with long names in a real 320px iframe. The desktop grid fills available width; the narrow case wraps names and keeps both amounts visible. These are representative visual checks, not exhaustive responsive acceptance.
...
- Still to validate before resolving the screen ticket: visual feedback on the compact planning revision, final responsive exceptions, representative final composition combinations in both schemes, keyboard/zoom/reflow, complete Goal-detail lifecycle/generated-Transaction examples and a precise final coverage record. The second-stage composition choices are recorded above. This is not a completed accessibility audit.
