...
- Registration does **not** chain into a second login call. `POST /api/auth/register` returns the same `{ token, user }` body a login does (the `pitaka` repo's "session on register" change), so `AuthService.register` maps it to a session grant and `Session` adopts it exactly as it adopts a sign-in — one request, no credentials replayed. An earlier build of the API returned no token here and forced the chained call; that is gone.
